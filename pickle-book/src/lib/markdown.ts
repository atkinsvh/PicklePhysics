...
import type { ChapterData, ChapterFrontMatter, TableOfContentsEntry } from "./types";
...
function restoreMath(html: string, mathMap: Map<string, string>): string {
  let result = html;
  for (const [key, mathHtml] of mathMap) result = result.replace(new RegExp(`<!--${key}-->`), mathHtml);
  return result;
}
